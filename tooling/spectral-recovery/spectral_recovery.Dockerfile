...
# Algorithm base path
ARG ALGORITHM_BASE="spectral_recovery_algo.main"
ENV ALGORITHM_BASE=${ALGORITHM_BASE}
ENV UV_PYTHON=3.12

# metadata
ARG SOLUTION="PEOPLE-ECCO Spectral Recovery"
...
COPY tooling/spectral-recovery/requirements.txt requirements.txt
RUN uv pip install --system -r requirements.txt

COPY bap/ ./hatfield/bap/
COPY spectral-recovery/src/ ./spectral_recovery_algo/