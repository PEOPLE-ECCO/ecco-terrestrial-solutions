#!/usr/bin/env python3
"""Plain Python CLI wrapper for the spectral-recovery workflow.

This wrapper avoids CWL by:
1) loading parameters from JSON,
2) authenticating an openEO connection,
3) invoking Algorithm.run from spectral-recovery/src/main.py,
4) writing a STAC catalog for produced items.
"""
...
def _find_repo_root(script_file: Path) -> Path:
    """Resolve repository root from either root or tooling entrypoint locations."""
    for candidate in (script_file.parent, *script_file.parents):
        if (candidate / "spectral-recovery" / "src" / "main.py").exists():
            return candidate
    raise FileNotFoundError(
        "Unable to locate repository root containing spectral-recovery/src/main.py "
        f"starting from {script_file}"
    )


def _load_algorithm_class(repo_root: Path):
    src_dir = repo_root / "spectral-recovery" / "src"
    ...
    if repo_root_str not in sys.path:
        # Allow imports like `from bap.BAP import ...` from repository root packages.
        sys.path.insert(0, repo_root_str)

    package_name = "_spectral_recovery_cli"
    ...
    if not hasattr(module, "Algorithm"):
        raise AttributeError(
            "Algorithm class not found in spectral-recovery/src/main.py"
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the hatfield spectral-recovery workflow without CWL."
    )
    parser.add_argument(
        "--parameters",
        required=True,
        help="Path to JSON parameter file (for example: tooling/spectral-recovery/spectral_recovery_run_parameters.json).",
    )
    parser.add_argument(
        "--output-dir",
        default="./runs",
    ...
    parser.add_argument(
        "--catalog-description",
        default="Spectral recovery CLI run",
