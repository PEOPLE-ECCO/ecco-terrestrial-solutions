#!/usr/bin/env python3
"""Plain Python CLI wrapper for the seasonal-sen workflow."""
...
def _find_repo_root(script_file: Path) -> Path:
    for candidate in (script_file.parent, *script_file.parents):
        if (candidate / "seasonal-sen" / "src" / "main.py").exists():
            return candidate
    raise FileNotFoundError(
        "Unable to locate repository root containing seasonal-sen/src/main.py "
        f"starting from {script_file}"
    )


def _load_algorithm_class(repo_root: Path):
    src_dir = repo_root / "seasonal-sen" / "src"
    ...
    package_name = "_seasonal_sen_cli"
    ...
    if not hasattr(module, "Algorithm"):
        raise AttributeError("Algorithm class not found in seasonal-sen/src/main.py")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the hatfield seasonal-sen workflow without CWL."
    )
    parser.add_argument(
        "--parameters",
        required=True,
        help="Path to JSON parameter file (for example: tooling/seasonal-sen/seasonal_sen_run_parameters.json).",
    )
    parser.add_argument(
        "--output-dir",
        default="./runs",
    ...
    parser.add_argument(
        "--cdse-client-id",
        default=None,
        help="Optional CDSE client id. If omitted, tries env OPENEO_AUTH_CLIENT_ID or CDSE_CLIENT_ID.",
    )
    parser.add_argument(
        "--cdse-client-secret",
        default=None,
        help="Optional CDSE client secret. If omitted, tries env OPENEO_AUTH_CLIENT_SECRET or CDSE_CLIENT_SECRET.",
    )
    return parser
...
def main() -> int:
    ...
    if client_id and client_secret:
        ...
        print(
            "No CDSE credentials provided; running in offline mode with precomputed BAPs."
        )
